...
class PeakFinder(object):
    ...
    def __closest_y(self, positions):
        ...
        for ri in np.arange(
                20):  # 15 out of all to reduce time (maximum 15 rows) from those that are closer comparing only "y"
            row = []
            diff_list = []
            try:
                cur_pos = self.__closest((min(positions)[0], max([item[1] for item in positions]) + 1),
                                         positions)  # we weight the y-value more than x-value
                row.append(cur_pos)
                for indx in np.where(np.array(positions) == cur_pos)[
                    0]:  # to check that we have x AND y the same (always together)
            ...
            if self.cg == 0 or self.cg == 1:
                positionsV = sorted(positions[0:15], key=lambda x: x[0],
                                    reverse=False)  # order by x the first 15 taken to avoid overtaking from other rows, careful with reverse true or false depending on sign
            elif self.cg == 3 or self.cg == 2:
                positionsV = sorted(positions[0:20], key=lambda x: x[0],
                                    reverse=False)  # order by x the first 20 taken to avoid overtaking from other rows, careful with reverse true or false depending on sign
            for pos in positionsV:  # 20 or 15 out of all to reduce time (maximum 20 peaks per row)
                ...
                if dist < low_dist and cur_pos[0] != pos[0]:
                    ...
                    stop = False
                    for indx in np.where(np.array(positions) == pos)[
                        0]:  # to check that we have x AND y the same (always together)
                        if not stop:
        ...
        for ch in range(len(dic_rows.keys())):
            check.append([dic_rows[ch][0][0][1],
                          dic_rows[ch][0][0][0]])  # we exchange the x and y only for the use of sorted(easier)
    ...
    def runPeakFinder(self):
        ...
        list1 = np.round(list1,
                         6).tolist()  # 6 is good because is the rounding for the normal numbers (without decimals from approx.
