...
class PeakFinder(object):            
    def __init__(self, cg, hist,bins,sigma,threshold,rmBackground,convIter,markov,mIter):
        ...
        @param cg = integer to indicate which cog algorithm is used
        @param hist = histogram containing the corresponding flood map
        @param bins = integer to indicate the number of bins used in the histograms [100 #same forall]
        @param sigma = float to indicate the accepted sigma to find peaks [3.3 works for 000,1.5 works fine for 111 and 100, 2.5 for 010]
        @param threshold = float to indicate the accepted threshold to find peaks [2 works for 000,4 in 111 for all and 6 for most(same for 010), 7 for 100]
        @param rmBackground = boolean to indicate whether the threshold is removed or not [True #forall]
        @param convIter = integer to indicate how many time the convolution is applied [200 #forall with more tends to reagroup peaks and with less some are missing]
        @param markov = boolean to indicate whether a smoothing function is applied [False #forall smoothing function to remark peaks, but it does not work for us, peaks with diffreent heights and sigmas]
        @param mIter = integer to indicate iterations of markov function [3 #forall no interest marov is false]
        """
    ...
    def __new_numpy1d_with_pointer(self, size):
        ...
        Input parameters: \
        size = int (it indicates how large the array should be)
    ...
    def __getPeaksR(self):
        ...
        Returns: \
        xycoord_arr = array with peak coordinates
        """
        binsx = self.bins
        binsy = self.bins
    ...
    def __closest(self, cur_pos, positions): 
        ...
        Input parameters: \
        cur_pos = point to compare with
        positions = list to look for the closest to cur_pos
        
        Returns: \
        closest_pos = coordinates of closest peak
        """
        low_dist = float('inf')
        closest_pos = None
    ...
    def closest_to_cent(self, dic_rows, Centers, roi):
        ...
        Input parameters: \
        dic_rows = dictionary containing rows from corresponding roi
        Centers = dictionary containing reference centers of corresponding roi
        roi = integer to indicate which region of interest is used
        
        Returns: \
        closest_pos = coordinates of closest peak to center
        roi = integer to indicate which roi is used
        """
        low_dist = float('inf')
        closest_pos = None
    ...
    def __closest_y(self, positions): 
        ...
        Input parameters: \
        positions = list with peaks to check
        
        Returns: \
        dic_rows = dictionary containing rows from corresponding roi
        positions = list with peaks not able to be assigned
        """
        #low_dist = float('inf')
        if self.cg == 0:
            low_dist = 0.15
