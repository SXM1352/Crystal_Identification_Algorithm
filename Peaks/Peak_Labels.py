...
class PeakLabels(object):
    ...
    def label(self, dic_palone, dic_rdefect, dic_crystal, dic_rows, dic_c): 
        #we define the function to set the labels to our peaks
        #dic_crystal[self.rows[dic_c[self.c_roi][1]][dic_c[self.c_roi][0]]]["center"] = closest
        rows_rest = []
        found = False
        ...
        for i in dic_rows.keys(): #we need to remove those rows that are classified and then loop again the rest
            ...
            i_crystal = i
            #print("dic_rows[i]",dic_rows[i_crystal])
            ...
            if ch_cent.size>0: #the peak is found (we need one to assure we have found the peak with x,y values and not only with x or y)
                ...
                if i_change:
                    i_crystal = i - n_alone
                if len(dic_rows[i][0]) == 1 and i != dic_rows.keys()[-1] and abs(np.diff([dic_rows[i][0][0][0],dic_rows[i_clo][0][ch_cent[0]][0]])) < 0.3: #to deal with the peaks that are alone
                    ...
                    #print(i)
                        continue
                if len(dic_rows[i][0]) < 3 and i != dic_rows.keys()[-1] and abs(np.diff([dic_rows[i][0][0][0],dic_rows[i_clo][0][ch_cent[0]][0]])) >= 0.3:
                ...
                if dop == 1: 
                    ...
                    if ch_cent[0] == 0:
                        ...
                        #print(i_crystal)
                        #print(dic_c[self.c_roi][1]+(i_crystal-i_clo))
                        #print(((dic_c[self.c_roi][0]*2)+2)+(en_j-ch[0]))
                        for en_j, j in enumerate(dic_rows[i][0]):
                            #dic_crystal[self.rows[dic_c[self.c_roi][1]+(i-i_clo)][dic_c[self.c_roi][0]+one_three[en_j]]]["center"][self.c_roi] = [j]
                            try:
                    ...
                    #                    print("works")
                     #                   print(rows[dic_c[self.c_roi][1]+(i_crystal-i_clo)][((dic_c[self.c_roi][0]*2)+2)+(en_j-ch[0])])
                                    #dic_crystal[self.rows[dic_c[self.c_roi][1]+(i-i_clo)][dic_c[self.c_roi][0]+en_j]]["valid"] = True
                            except:
                      #          print("Problem choosing the correct row.")
                                row_defect = True       
                    dop = 0
                else: # and dop == 0: 
                    #print("Ahora NOOOOOO!!!!,bitte. #check to see whether it works when we are not at the edges!!! ch[0] does not have to be the correct relative position (closest to x=12,etc?) within the row for the other rows with 3rd peaks (CHECK!!!)")
                    for en_j, j in enumerate(dic_rows[i][0]):
                        #print((en_j-ch[0]))
                        #print(dic_c[self.c_roi][0]+(en_j-ch[0]))
        ...
        for h in rows_rest: #remember that here we have h!
            ...
            h_crystal = h
            #print("dic_rows[h]",dic_rows[h_crystal])
            ...
            if len(dic_rows[h][0]) < 3 and h != 0 and abs(np.diff([dic_rows[h][0][0][0],dic_rows[i_clo][0][ch_cent[0]][0]])) >= 0.3: #we check it is not an important peak
                if dic_c[self.c_roi][0] == 0 and dic_rows[h][0][ch[0]] == dic_rows[h][0][0] and len(dic_rows[i][0]) > 1:
                    ...
                    if (self.c_roi in dic_palone.keys()):
                        dic_palo[dic_palone[self.c_roi].keys()[0]] = dic_palone[self.c_roi].values()
                        dic_palone[self.c_roi] = dic_palo
                    else:
                        dic_palone[self.c_roi] = dic_palo
                    dic_palo = {}
            ...
            if dop == 1: 
                if ch_cent[0] == 0:
                    ...
                    for en_j, j in enumerate(dic_rows[h][0]):
                        #dic_crystal[self.rows[dic_c[self.c_roi][1]+(h-i_clo)][dic_c[self.c_roi][0]+one_three[en_j]]]["center"][self.c_roi] = [j]
                        try:
                            ...
                            if len(dic_rows[h][0]) > self.dop_min_cent and len(dic_rows[h][0]) < self.dop_max_cent: #before 7 and 14(010)/7,15(100)
                                ...
                                #dic_crystal[self.rows[dic_c[self.c_roi][1]+(h-i_clo)][dic_c[self.c_roi][0]+en_j]]["valid"] = True
