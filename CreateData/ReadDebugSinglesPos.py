...
def __parallel_apply_line_by_line(input_file_path, chunk_size_factor, num_procs, skiplines, func_apply, func_args,
    ...
    jobs = __chunkify_file(input_file_path, 1024 * 1024 * chunk_size_factor,
                           skiplines)  # jobs are the lines of the file

    jobs = [list(x) + [func_apply] + func_args for x in
            jobs]  # x is each line, jobs will be now a list with each slected line and the function that will be applied to each line
...
# def __save_data_hdf5(folder, dic_HVD, name):
#     array_to_save = []
#     for cluster in dic_HVD.keys():
#
#     with open('{}/{}.pickle'.format(folder, name), 'wb') as handle:
#         pickle.dump(dic_HVD, handle, protocol=pickle.HIGHEST_PROTOCOL)
#     print('saved.')
...
for cluster in list_save_dic["dicpos_000"].keys():
    cogRef.append([int(list_save_dic["dicval_000"][cluster]), int(list_save_dic["dicval_100"][cluster]),
                   int(list_save_dic["dicval_010"][cluster]), int(list_save_dic["dicval_111"][cluster])])
