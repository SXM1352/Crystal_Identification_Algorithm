...
for cluster in list_save_dic_crt['stack_id'].keys():
    if list_save_dic_crt['stack_id'][cluster] == stack_id_cal:
        try:
            ...
            print("Fehler bei cluster: ", cluster)
        ...
        try:
            ...
            print("Fehler bei cluster", cluster)
#
pv000Ref_cal = np.array(pv000Ref_cal[:len(pv000Ref_cal)-2])
pv100Ref_cal = np.array(pv100Ref_cal[:len(pv100Ref_cal)-2])
pv010Ref_cal = np.array(pv010Ref_cal[:len(pv010Ref_cal)-2])
pv111Ref_cal = np.array(pv111Ref_cal[:len(pv111Ref_cal)-2])
print("Arrays ready")
pv000Ref_coinc = np.array(pv000Ref_coinc[:len(pv000Ref_coinc)-2])
pv100Ref_coinc = np.array(pv100Ref_coinc[:len(pv100Ref_coinc)-2])
pv010Ref_coinc = np.array(pv010Ref_coinc[:len(pv010Ref_coinc)-2])
pv111Ref_coinc = np.array(pv111Ref_coinc[:len(pv111Ref_coinc)-2])
print("Arrays ready")
