...
def __parallel_apply_line_by_line(start, stop, cores, input_file_path, chunk_size_factor, num_procs, skiplines, func_apply, func_args,
                                fout=None):
    """
    ...
    print("Maximal Stop value = ", len(jobs))
    print("Start: ", start,        "Stop: ", stop)
    num_parallel = cores

    for i in range(start, stop, num_parallel): #max 1040
        print("Chunk start = ", i)
        t1 = time.time()
...
def create_hdf5(start, stop, cores):
    #############Einstellungen############
    start = start
    cores = cores
    stop = stop
    ...
    for i_s in list_save.keys():
        ...
        if len(index) > 1:
            ...
            outp, list_save_dic[i_s] = __parallel_apply_line_by_line(start, stop, cores,
                pathtodirectoryRead + "{}{}".format(stack_id, file_type),
                100, 8, 0, __index_inline_crt, [index[0], index[1]], fout=None)
        else:
            outp, list_save_dic[i_s] = __parallel_apply_line_by_line(start, stop, cores,
                pathtodirectoryRead + "{}{}".format(stack_id, file_type),
                100, 8, 0, __index_inline, [index[0]], fout=None)
    ...
    with h5py.File('{}cogRef_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
    ...
    with h5py.File('{}cog000ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events, 2), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = cog000Ref[i: i + dset.chunks[0]]

    with h5py.File('{}cog100ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events, 2), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = cog100Ref[i: i + dset.chunks[0]]

    with h5py.File('{}cog010ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events, 2), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = cog010Ref[i: i + dset.chunks[0]]

    with h5py.File('{}cog111ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events, 2), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = cog111Ref[i: i + dset.chunks[0]]

    with h5py.File('{}pv000ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events,), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = pv000Ref[i: i + dset.chunks[0]]

    with h5py.File('{}pv100ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events,), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = pv100Ref[i: i + dset.chunks[0]]

    with h5py.File('{}pv010ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events,), chunks=True)

        for i in range(0, n_events, dset.chunks[0]):
            dset[i: i + dset.chunks[0]] = pv010Ref[i: i + dset.chunks[0]]

    with h5py.File('{}pv111ref_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
        dset = f.create_dataset("data", (n_events,), chunks=True)
    ...
    with h5py.File('{}photons_coinc_{}_{}.hdf5'.format(pathtodirectorySave_hdf, start, stop), 'w') as f:
